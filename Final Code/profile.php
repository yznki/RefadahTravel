...
$sql = "SELECT * FROM customer WHERE emailAddress = :emailadd";

$stmt = $pdo->prepare($sql);
$stmt->bindParam(':emailadd', $_SESSION['email'], PDO::PARAM_STR);
$stmt->execute();
$image = $stmt->fetchAll();
...
<head>
  ...
  <title>Profile - Refadah Travel & Tourism</title>
...
<body>
  ...
  <div class="websitePage">
    ...
    <div class="contentsection">
      ...
      require 'connectingtodatabase.php';
      $sql = "SELECT * FROM customer_package_info WHERE Email = :emailadd ORDER BY Departure_Date;";
      ...
      ?>
      <div class="userinfo">
        <img src="<?php echo $image[0]['userImage'] ?>" alt="Profile Picture" class="pp" style="object-fit: cover">
        <div class="username">
          <h1><?php echo $_SESSION['username'] . ' ' . $_SESSION['lname'] ?></h1>
        </div>
        <div class="useremail">
          <h5><?php echo $_SESSION['email'] ?></h5>
        </div>
        <div class="edit"> <a href="editprofile.php"><button name="edit" class="editbutton">Edit Profile</button></a></div>

      </div>
      
      <hr class="mobileSeperate">
      <div class="packagesh mobilehead"><h1>Customer Packages</h1></div>

      <div class="packages">
        <div class="packagesh webhead"><h1>Customer Packages</h1></div>
        <?php foreach ($data as $row) { ?>
          <div class="packagecontent">
            <img src="<?php echo $row['Image'] ?>" class="packageimage" alt="PackageImage" style="object-fit: cover">
            <div class="packageinfo">
              <div class="packagetitle">
                <h1 class="citytitle"><?php echo $row['City'] . ', ' . $row['Country'] ?></h1>
                <p class="packagedeparture"><span class="departstext">Departs : </span><?php echo $row['Departure_Date'] ?></p>
              </div>
            </div>
            <a href=<?php echo "delete.php?package=" . $row['PackageID'] . "&id=" . $row['ID'];
                    $_SESSION['delete'] = 1; ?>><button name="deletepack" class="deletebutton"><i class="fa-solid fa-trash-can"></i> <span class="deltext">Delete</span></button></a>
